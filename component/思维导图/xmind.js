  ...
  functions: {
    ...
    saveAsImage: function (root) {
      ...
      var padding = this.style.centralSpacing,
        ...
        canvas = this.$canvas, dpr = this.$dpr;
      canvas.style.width = (canvas.width = Math.round(width * dpr)) / dpr + "px";
      canvas.style.height = (canvas.height = Math.round(height * dpr)) / dpr + "px";
      ...
      this.imageData.setImageData(0, 0, width, height).saveAsImage(root.getValue());
      canvas.style.width = (canvas.width = this.$width * dpr) / dpr + "px";
      canvas.style.height = (canvas.height = this.$height * dpr) / dpr + "px";
      this._$resetTransform();
      this._$resetStyle();
      this.point.add(-offsetX, -offsetY);
      this.draw();
